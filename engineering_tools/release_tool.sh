...
function replace_docker_image_versions {
  ...
  sed -i "s|usdotfhwastoldev|usdotfhwastol|; s|usdotfhwastolcandidate|usdotfhwastol|; s|:[0-9]*\.[0-9]*\.[0-9]*|:$IMAGE_VERSION|g; s|:CARMA[a-zA-Z]*_[0-9]*\.[0-9]*\.[0-9]*|:$IMAGE_VERSION|g; s|:carma-[a-zA-Z]*-[0-9]*\.[0-9]*\.[0-9]*|:$IMAGE_VERSION|g; s|:develop|:$IMAGE_VERSION|g; s|:battista|:$IMAGE_VERSION|g" \
      $FILE
}
...
function update_version_numbers {
    ...
    if [[ $REPOSITORY == "carma-config/" ]]; then
        ...
        for config in */; do
            if [[ -f $config/docker-compose.yml ]]; then
                ...
                sed -i "s|usdotfhwastoldev|usdotfhwastol|; s|usdotfhwastolcandidate|usdotfhwastol|; s|:[0-9]*\.[0-9]*\.[0-9]*|:$TARGET_VERSION_ID|g; s|:CARMA[a-zA-Z]*_[0-9]*\.[0-9]*\.[0-9]*|:$TARGET_VERSION_ID|g; s|:carma-[a-zA-Z]*-[0-9]*\.[0-9]*\.[0-9]*|:$TARGET_VERSION_ID|g; s|:develop|:$TARGET_VERSION_ID|g; s|:battista|:$TARGET_VERSION_ID|g" \
    ...
    if [[ -f docker/checkout.bash ]]; then
        sed -i "/\$BRANCH/ b; /usdot-fhwa-stol/ s|--branch .*|--branch $TARGET_VERSION_ID|g; /usdot-fhwa-stol/ s|battista|$TARGET_VERSION_ID|g" \
